  ...
  return (
    <>
      ...
      <nav className="sticky top-0 z-50 h-20 w-full bg-white shadow-md">
        ...
        <div className="flex flex-row justify-between h-full w-full items-center px-4  2xl:px-16">
          ...
          <div className="hidden md:flex flex-row gap-2 pl-3">
            {socialLink.map((items) => {
              return (
                <Link key={items.link} target="_blank" href={items.link}>
                  <items.social size={25} className={`${items.color} `} />
                </Link>
              );
            })}
          </div>
          {/* social links end here */}
          <div onClick={handleClick} className="md:hidden pr-13">
            {" "}
        ...
        {/* nav menu for mobile screens starts here */}
        <div
          className={
            isMenuOpen
              ? "md:hidden fixe₫ right-0 top-25 w-[250px] bg-slate-500 rounded-md absolute z-50"
              : "fixed right-[-100%]"
          }
        >
          <ul className="flex flex-col mt-4 p-3 ">
            {menuLink.map(({ page, name, icons }) => (
              <li
                ...
                className={`${
                  router == page
                    ? "bg-duskblue-500 text-white"
                    : "hover:text-duskblue-300"
                } `}
              >
                <Link
                  onClick={(e) => setMenuOpen(false)}
                  onScroll={(e) => setMenuOpen(false)}
        ...
        {/* nav menu for mobile screens ends here  */}
      </nav>
    </>
  );
};
